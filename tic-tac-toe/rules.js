...
function isEmpty(_str) {
	console.log(!_str || 0 === _str.length);
...
function begin_play(){
	...
	if(isEmpty(name1.value) || isEmpty(name2.value)){
		alert("Player 1 and Player 2 need names!");
...
*/
function play() {
	var placement = document.getElementById("move_text_id").value;
	var i;

	if(game_started()){
		for(i=0; i<table_ids.length; i++){
			if(placement == table_ids[i]){
				if(board_state[i] == -1){
		...
		(board_state[2] == board_state[4] && board_state[4] == board_state[6])){
			if(board_state[4] == 1){
				alert("Winner! O player");
				reset_play();
			}else if (board_state[4] == 0){
				alert("Winner! X player");
				reset_play();
			}
